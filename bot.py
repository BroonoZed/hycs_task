...
def looks_like_question(text: str) -> bool:
    ...
    if any(k.lower() in lowered for k in QUESTION_KEYWORDS):
        return True
    # 常见订单号特征
    if re.search(r"(?i)\b(order|订单|mch|#)[-_:#\s]*[a-z0-9]{4,}\b", s):
        return True
    return False
...
def list_open_tasks(limit: int = 30) -> list[sqlite3.Row]:
    with closing(db()) as conn:
        return conn.execute(
            "SELECT * FROM tasks WHERE status='OPEN' ORDER BY id DESC LIMIT ?",
            (max(1, min(limit, 100)),),
        ).fetchall()
...
def format_task_card(task_id: int, chat_id: int, msg_id: int, asker: str, text: str, created_at_iso: str) -> str:
    created = datetime.fromisoformat(created_at_iso).astimezone(UTC8).strftime("%Y-%m-%d %H:%M:%S")
    src_link = f"https://t.me/c/{str(chat_id)[4:]}/{msg_id}" if str(chat_id).startswith("-100") else f"chat:{chat_id}#{msg_id}"
    return (
        f"🧩 *客服任务 #{task_id}*\n"
        f"状态: OPEN\n"
        f"来源: `{chat_id}` / msg `{msg_id}`\n"
        f"提问人: {asker}\n"
        f"创建: {created} (UTC+8)\n"
        f"原消息: {src_link}\n"
        f"问题: {summarize(text, 220)}\n\n"
        f"命令: `/task_done {task_id}`  `/task_bind {task_id} 订单号`  `/task_order {task_id}`"
    )
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg.text:
        return
    if not looks_like_question(msg.text):
        return

    task_id = insert_task(chat.id, msg.message_id, user.id, user.full_name or user.username or str(user.id), msg.text)
    if not task_id:
        return

    card = format_task_card(task_id, chat.id, msg.message_id, user.full_name or user.username or str(user.id), msg.text, now_str())
    sent = await context.bot.send_message(chat_id=TASK_CHANNEL_ID, text=card, parse_mode=ParseMode.MARKDOWN)
    set_task_channel_msg(task_id, sent.message_id)


async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("hycs_task 已启动。")
...
async def cmd_task_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id if update.effective_user else None):
        ...
        return
    rows = list_open_tasks(limit=20)
    if not rows:
        await update.message.reply_text("当前无 OPEN 任务")
        return
    lines = ["OPEN任务："]
    for r in rows:
        lines.append(f"#{r['id']} | src={r['source_chat_id']}:{r['source_message_id']} | remind={r['remind_count']} | {summarize(r['question_text'], 60)}")
...
async def cmd_task_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1 or not context.args[0].isdigit():
        ...
        return
    task = get_task(int(context.args[0]))
    if not task:
        await update.message.reply_text("任务不存在")
        return
...
async def job_overdue(context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        await context.bot.send_message(
            ...
            text=f"⏰ 任务 #{r['id']} 已超时 {mins} 分钟（SLA={TASK_SLA_MINUTES}m），请尽快处理。",
        )
        inc_remind(int(r["id"]))
